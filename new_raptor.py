...
class NewRaptor:
    ...
    def run(self):
        ...
        if self.first_start == True:
            ...
            self.dlg.cmbSpecies.currentTextChanged.connect(self.onSpeciesChange)
        ...
        if result:
            ...
            # add new nest
            ftNest = QgsFeature()
            ftNest.setFields(lyrNests.fields(), True)  # Assure la structure des champs et initialise les attributs
            ...
            lyrNests.dataProvider().addFeatures([ftNest])
            lyrNests.reload()
            # create buffer
            buffer = geom.buffer(valBuffer, 10)
            ftNest.setGeometry(buffer)
            lyrBuffer.dataProvider().addFeatures([ftNest])
            lyrBuffer.reload()
    ...
    def onSpeciesChange(self, species):
